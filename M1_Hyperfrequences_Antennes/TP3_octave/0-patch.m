...
%patch.L = 28.05151;		%typon
patch.L = 28.5288;
patch.W = 37.0218;%*1.53;

line.L = 0;%.1;
%line.L = 16.621;		%%mres/60
line.W = 2.8892;		%%sres/20
%line.L = 18.0658;		%%mres/50
%line.W = 0.265953;		%%sres/20
...
CSX = AddBox(CSX, 'Line', 2, [-line.W/2, -patch.L/2, 0], [line.W/2, -patch.L/2-line.L, copper.h]);
...
%%%% PORT
start = [-feed.W/2, -patch.L/2-line.L+0.1, 0];
stop  = [feed.W/2, -patch.L/2-line.L-0.1, -substrate.h];
...
%figure
%plot( freq/1e6, sqrt((real(Zin)*real(Zin))+(imag(Zin)*imag(Zin))), 'k-', 'Linewidth', 2 );
%grid on
%title( 'feed point impedance' );
%xlabel( 'frequency f (MHz)' );
%ylabel( 'impedance Zin (Ohm)' );
...
%f_25_ind = 251;
%f_245    = 239;
%n_ = 1000; %// Define number of points on circle
%theta_ = linspace(0, 2*pi, n_);
%x_ = cos(theta_);
%y_ = sin(theta_);
%
%figure
...
%plot(x_,y_, 'k-');
%hold on
%plot(x_/2-0.5,y_/2, 'k-');
%plot(x_/2+0.5,y_/2, 'k-');
%hold on
%plot(real(s11),imag(s11), 'b--', 'Linewidth', 1);
%hold on
%plot(real(s11(f_24_ind:f_25_ind)),imag(s11(f_24_ind:f_25_ind)), 'r-', 'Linewidth', 2);
%hold on
%plot(real(s11(f_res)),imag(s11(f_res)), 'b', 'marker', 'o');
%hold on
%plot(real(s11(f_24_ind)),imag(s11(f_24_ind)), 'b', 'marker', 'x');
%hold on
%plot(real(s11(f_25_ind)),imag(s11(f_25_ind)), 'b', 'marker', '+');
%hold on
%axis equal;
%grid on
%title( 'S11, x = 2.4 + = 2.5Ghz, o = 2.45Ghz' );
%drawnow
...
%figure
%D_log = nf2ff.E_norm{1}/max(max(nf2ff.E_norm{1}));
%D_log = D_log * nf2ff.Dmax;
%polar( nf2ff.theta, D_log(:,1) ,'k-', nf2ff.theta, D_log(:,2) ,'r-' );
%drawnow


%E_ff_phi = nf2ff.E_phi{1};
%E_ff_theta = nf2ff.E_theta{1};
...
%figure
%polar([-180:2:180]'*pi/180,abs(nf2ff.E_phi{1}(:,1))/max(abs(nf2ff.E_phi{1}(:,1))));
%hold on
%polar([0 90]'*pi/180,abs(nf2ff.E_phi{1}(:,1))/max(abs(nf2ff.E_phi{1}(:,1))),'r--')
