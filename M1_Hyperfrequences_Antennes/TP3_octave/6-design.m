...
% w0
patch2.W = patch.W/2-line.W/2-line.W*1;
% y0
%patch2.L = 10.112*1;		% 256.5 -> 50R
patch2.L = 8.151*1;			% 256.5 -> 100R
%patch2.L = 7.7943*1;		% 117
%patch2.L = 7.1322*1;		% L/4
%patch2.L = 7.1322*0.99;	% L/4
%patch2.L = 6.6012*1;		% 89.551
%patch2.L = 4.7548*1;		% L/6
...
CSX = AddBox(CSX, 'Line', 2,...
	[-line.W/2, -patch1.L/2, 0],...
	[line.W/2, -patch1.L/2-line.L, copper.h]);
CSX = AddBox(CSX, 'Patch1', 2,...
	[-patch.W/2, -patch1.L/2 0],...
	[patch.W/2, patch1.L/2, copper.h]);
CSX = AddBox(CSX, 'Patch2', 2,...
	[-patch.W/2, -patch1.L/2 0],...
	[-patch.W/2+patch2.W, -patch1.L/2-patch2.L, copper.h]);
...
mesh.x = [ ...
	...
	(patch.W/2)+(2*mres/3) ...
	(patch.W/2)-(mres/3) ...
	(-patch.W/2)+(patch2.W)+(2*mres/3) ...
	(-patch.W/2)+(patch2.W)-(mres/3) ...
	(patch.W/2)-(patch2.W)-(2*mres/3) ...
	(patch.W/2)-(patch2.W)+(mres/3) ...
	(0) ...
	];
mesh.y = [ ...
	(-substrate.L/2) ...
	(substrate.L/2) ...
	(-patch1.L/2-patch2.L)-(2*mres/3) ...
	(-patch1.L/2-patch2.L)+(mres/3) ...
	(patch1.L/2)+(2*mres/3) ...
	(patch1.L/2)-(mres/3) ...
	(-patch1.L/2)-(2*mres/3) ...
	(-patch1.L/2)+(mres/3) ...
%	(-patch1.L/2-line.L)-(2*mres/3) ...
%	(-patch1.L/2-line.L)+(mres/3) ...
	(-patch1.L/2-line.L) ...
	(0) ...
	];
...
%%%% PORT
start = [-feed.W/2, -patch1.L/2-line.L+0.1, 0];
stop  = [feed.W/2, -patch1.L/2-line.L-0.1, -substrate.h];
